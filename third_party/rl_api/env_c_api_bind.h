...
// Version of Bind which accepts a raw pointer and a custom release method.
template <class T>
void Bind(T* env, void (*release)(void*), EnvCApi* api, void** context);
...
void Bind(std::unique_ptr<T> env, EnvCApi* api, void** context) {
  Bind(
      env.release(), [](void* ptr) { delete static_cast<T*>(ptr); }, api,
      context);
}
