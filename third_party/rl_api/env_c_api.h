...
//
// An observation consists of a multidimensional array of either bytes, int32s,
// int64s or doubles. This "shape" of the observation is described by an
// "observation spec", which is part of the observation. (However, the API may
// additionally expose a fixed spec that is always valid, which simplifies
// observation handling for the caller.) The environment returns a tuple of
// observations, whose size can be queried through the API.
//
...
enum EnvCApi_ObservationType_enum {
  ...
  EnvCApi_ObservationInt32s,
  EnvCApi_ObservationInt64s
};
typedef enum EnvCApi_ObservationType_enum EnvCApi_ObservationType;

// An observation is either a string, a scalar, or a multidimensional array
// of numbers.
//
// The data type of the scalar or the array elements is described by 'type'.
// If 'dims' is zero, then the observation is a scalar and 'payload' points
// to a single value of type 'type'; otherwise, 'payload' points to a flat
// array of size
//
//     shape[0] * shape[1] * ... * shape[dims - 1],
//
// laid out in strides in row-major (C-style) order.
//
// If the 'type' is 'EnvCApi_ObservationString', then the 'dims' shall be 1 and
// 'shape[0]' shall be the length of the string.
//
// In any case, ['shape', 'shape + dims') shall be a valid range.
...
struct EnvCApi_s {
  ...
  //
  // If 'dims' is -1 in the resulting spec, the shape of this observation has to
