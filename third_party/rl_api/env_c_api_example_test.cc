...
TEST(EnvCApiExampleTest, WriteProperty) {
  ...
  EXPECT_EQ(env_c_api.write_property(context, "prop2_writeonly", "NewValue2"),
            EnvCApi_PropertyResult_Success);
  env_c_api.release_context(context);
}
...
TEST(EnvCApiExampleTest, WritePropertyFail) {
  ...
  EXPECT_EQ(env_c_api.read_property(context, "prop3_readonly", &value),
            EnvCApi_PropertyResult_Success);
  EXPECT_STREQ(value, "3");
  EXPECT_EQ(env_c_api.write_property(context, "prop3_readonly", "NewValue3"),
            EnvCApi_PropertyResult_PermissionDenied);
  value = nullptr;
  EXPECT_EQ(env_c_api.read_property(context, "prop3_readonly", &value),
            EnvCApi_PropertyResult_Success);
...
TEST(EnvCApiExampleTest, ObservationInt32s) {
  EnvCApi env_c_api;
  void* context;
  env_c_api_example_connect(&env_c_api, &context);
  EXPECT_EQ(0, env_c_api.init(context));
  int observation_count = env_c_api.observation_count(context);
  EXPECT_GT(observation_count, 0);
  std::vector<int> int32s_ids;
  for (int idx = 0; idx < observation_count; ++idx) {
    EnvCApi_ObservationSpec spec;
    env_c_api.observation_spec(context, idx, &spec);
    if (spec.type == EnvCApi_ObservationInt32s) {
      int32s_ids.push_back(idx);
      ASSERT_EQ(1, spec.dims);
      EXPECT_EQ(4, spec.shape[0]);
    }
  }

  env_c_api.start(context, /*episode=*/1, /*seed=*/1234);
  ASSERT_EQ(1, int32s_ids.size());

  int int32s_id = int32s_ids.front();
  {
    EnvCApi_Observation obs;
    env_c_api.observation(context, int32s_id, &obs);
    EXPECT_EQ(EnvCApi_ObservationInt32s, obs.spec.type);
    ASSERT_EQ(1, obs.spec.dims);
    EXPECT_EQ(4, obs.spec.shape[0]);
    std::vector<std::int32_t> integer_observation(
        obs.payload.int32s, obs.payload.int32s + obs.spec.shape[0]);

    for (std::int32_t v : integer_observation) {
      EXPECT_EQ(127, v);
    }
  }
  env_c_api.act_discrete(context, kDiscreteActions);
  ...
  EXPECT_EQ(0.0, reward);
  {
    ...
    ASSERT_EQ(1, obs.spec.dims);
    EXPECT_EQ(4, obs.spec.shape[0]);
    std::vector<int32_t> integer_observation(
        obs.payload.int32s, obs.payload.int32s + obs.spec.shape[0]);
...
TEST(EnvCApiExampleTest, ObservationInt64s) {
  EnvCApi env_c_api;
  void* context;
  env_c_api_example_connect(&env_c_api, &context);
  EXPECT_EQ(0, env_c_api.init(context));
  int observation_count = env_c_api.observation_count(context);
  EXPECT_GT(observation_count, 0);
  std::vector<int> int64s_ids;
  for (int idx = 0; idx < observation_count; ++idx) {
    EnvCApi_ObservationSpec spec;
    env_c_api.observation_spec(context, idx, &spec);
    if (spec.type == EnvCApi_ObservationInt64s) {
      int64s_ids.push_back(idx);
      ASSERT_EQ(1, spec.dims);
      EXPECT_EQ(6, spec.shape[0]);
    }
  }

  env_c_api.start(context, /*episode=*/1, /*seed=*/1234);
  ASSERT_EQ(1, int64s_ids.size());

  int int64s_id = int64s_ids.front();
  {
    EnvCApi_Observation obs;
    env_c_api.observation(context, int64s_id, &obs);
    EXPECT_EQ(EnvCApi_ObservationInt64s, obs.spec.type);
    ASSERT_EQ(1, obs.spec.dims);
    EXPECT_EQ(6, obs.spec.shape[0]);
    std::vector<std::int64_t> integer_observation(
        obs.payload.int64s, obs.payload.int64s + obs.spec.shape[0]);

    for (std::int64_t v : integer_observation) {
      EXPECT_EQ(127, v);
    }
  }
  env_c_api.act_discrete(context, kDiscreteActions);
  ...
  EXPECT_EQ(0.0, reward);
  {
    ...
    ASSERT_EQ(1, obs.spec.dims);
    EXPECT_EQ(6, obs.spec.shape[0]);
    std::vector<int64_t> integer_observation(
        obs.payload.int64s, obs.payload.int64s + obs.spec.shape[0]);
