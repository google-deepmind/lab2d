...
lua::NResultsOr Scale(lua_State* L) {
  ...
  if (source == nullptr || !source->tensor_view().IsContiguous() ||
    ...
    return absl::StrCat("[image.scale] -  Arg 1 \"", lua::ToString(L, 1),
                        "\" - Invalid source image");
  }
  std::size_t target_rows;
  if (!lua::Read(L, 2, &target_rows)) {
    return absl::StrCat("[image.scale] - Arg 2 \"", lua::ToString(L, 2),
                        "\" - Invalid number of output rows");
  }
  std::size_t target_cols;
  if (!lua::Read(L, 3, &target_cols)) {
    return absl::StrCat("[image.scale] - Arg 3 \"", lua::ToString(L, 3),
                        "\" - Invalid number of output columns");
  }
  std::string mode = "bilinear";
  if (IsTypeMismatch(lua::Read(L, 4, &mode))) {
    return absl::StrCat("[image.scale] - Arg 4 \"", lua::ToString(L, 4),
...
void ApplyPattern(unsigned char* source_start,
                  const unsigned char* pattern_start, std::size_t pattern_span,
                  std::size_t pixel_count, unsigned char r1, unsigned char g1,
