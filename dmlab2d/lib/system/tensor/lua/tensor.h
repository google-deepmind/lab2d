...
#ifndef DMLAB2D_LIB_SYSTEM_TENSOR_LUA_TENSOR_H_
#define DMLAB2D_LIB_SYSTEM_TENSOR_LUA_TENSOR_H_
...
class LuaTensor : public lua::Class<LuaTensor<T>> {
 ...
 public:
  static void Register(lua_State* L) {
    const typename Class::Reg methods[] = {
        ...
        {"isContiguous", &Class::template Member<&LuaTensor<T>::IsContiguous>},
    ...
    if (lua::Read(L, 1, &table)) {
      ...
      if (keys.empty()) {
        if (table.ArraySize() == 0) {
          LuaTensor::CreateObject(L, ShapeVector{0}, std::vector<T>{});
          return 1;
        } else {
  ...
  bool IsValidObject() { return storage_validity_->IsValid(); }
 ...
 private:
  ...
  static lua::NResultsOr CreateFromFile(lua_State* L, lua::TableRef file_args) {
    ...
    if (!ifs.Success()) {
      return absl::StrCat("[Tensor.CreateFromFile] Failed to open file, name: ",
                          name);
    }

    std::size_t file_size;
    if (!ifs.GetSize(&file_size)) {
      return absl::StrCat("[Tensor.CreateFromFile] Failed to read file, name: ",
                          name);
    }
    ...
    if (IsTypeMismatch(parse_result)) {
      return absl::StrCat(
          "[Tensor.CreateFromFile] 'numElements' must be a "
      ...
      return absl::StrCat(
          ...
          name, ", numElements: ", num_elements,
          ", max numElements: ", max_num_elements, ", offset: ", offset,
          ", file size: ", file_size);
    }
  ...
  // Creates LuaTensor at the top of the Lua stack copying the metatable from
  // the first element of the Lua stack. Only call when sure that this metatable
  // will be the correct one. (I.e member functions returning new Tensors of
  // the same type.)
  template <typename... Args>
  static LuaTensor* CreateObjectSameClass(lua_State* L, Args&&... args) {
    void* userdata = lua_newuserdata(L, sizeof(LuaTensor));
    lua_getmetatable(L, 1);
    lua_setmetatable(L, -2);
    return new (userdata) LuaTensor(std::forward<Args>(args)...);
  }
  ...
  static void ToLuaTable(lua_State* L, const tensor::TensorView<T>& view) {
    ...
    if (shape.empty()) {
      lua::Push(L, view.storage()[view.start_offset()]);
      return;
    }
  ...
  lua::NResultsOr ApplyIndexed(lua_State* L) {
    ...
    tensor_view_.ForEachIndexedMutable([L, &err](const ShapeVector& index,
                                                 T* value) {
      lua_pushvalue(L, 2);
      lua::Push(L, *value);
      ...
      if (result.ok()) {
        if (result.n_results() > 0) {
          if (IsTypeMismatch(lua::Read(L, -result.n_results(), value))) {
            err = absl::StrCat("Invalid arg '",
  ...
  lua::NResultsOr Clamp(lua_State* L) {
    ...
    if (min_value != std::numeric_limits<T>::lowest() &&
      ...
      tensor_view_.ForEachMutable(
          [min_value](T* value) { *value = std::max(min_value, *value); });
    } else if (max_value != std::numeric_limits<T>::max()) {
      tensor_view_.ForEachMutable(
          [max_value](T* value) { *value = std::min(*value, max_value); });
    }
    lua_settop(L, 1);
    return 1;
  }
  ...
  lua::NResultsOr Reshape(lua_State* L) {
    ...
    if (!IsFound(lua::Read(L, 2, &new_shape))) {
      return "Arg1 must be a table of integers.";
    } else if (!result.Reshape(std::move(new_shape))) {
  ...
  lua::NResultsOr Clone(lua_State* L) {
    ...
    CreateObjectSameClass(L, tensor_view_.shape(), std::move(storage));
    return 1;
  }
  ...
  lua::NResultsOr MMul(lua_State* L) {
    if (LuaTensor* rhs = LuaTensor::ReadObject(L, 2)) {
      ...
      std::vector<T> storage(Layout::num_elements(shape));
      LuaTensor* prod =
          CreateObjectSameClass(L, std::move(shape), std::move(storage));
  ...
  lua::NResultsOr IsContiguous(lua_State* L) {
    lua::Push(L, tensor_view_.IsContiguous());
    return 1;
  }
...
#endif  // DMLAB2D_LIB_SYSTEM_TENSOR_LUA_TENSOR_H_
