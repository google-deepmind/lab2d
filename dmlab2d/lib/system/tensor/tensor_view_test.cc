...
TEST(TensorViewTest, ReshapeToScalar) {
  ShapeVector shape = {1, 1, 1};
  std::vector<float> storage = {10.f};
  TensorView<float> view(Layout(std::move(shape)), storage.data());
  EXPECT_TRUE(view.Reshape({}));
  EXPECT_THAT(view.stride(), IsEmpty());
  EXPECT_THAT(view.shape(), IsEmpty());
  EXPECT_EQ(storage[0], 10.0f);
}
...
TEST(TensorViewTest, TestAssignNonContig) {
  ...
  view2.ForEach([](float val) { EXPECT_EQ(val, 2.0f); });
}
...
TEST(TensorViewTest, TestEqual) {
  ...
  std::vector<int> storage1 = MakeSequence<int>(Layout::num_elements(shape));
  TensorView<int> view1(Layout(shape), storage1.data());
